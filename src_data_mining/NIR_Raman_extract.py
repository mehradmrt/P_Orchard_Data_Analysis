    ...
    for i in range(len(nirdict)):
        ...
        for filename in files:
            ...
            if i==0 and counter==0:
                ...
                for fake in range(len(samdict)):
                    
                    # dfn = (mdf[mdf['test_number']=='T5'][mdf['tree_id']==8]\
                    #        [mdf['sample_number']=='S1']).iloc[:,4:]
                    dfn = pd.read_csv(filename, skiprows=32, delimiter='\t')
                    dfn = pd.DataFrame().reindex_like(dfn)
                    dfn.insert(0,"sample_number",samdict[fake])
                    dfn.insert(0,"tree_id",tree_idx)
                    dfn.insert(0,"test_number",tdays[i])
                    print(dfn)
                    mdf = pd.concat([mdf,dfn])
                tree_idx +=1
...
T5correct = ['B11L34','B11L33','B11L32','B8L6','B8L7','B8L8','B5L14',\
            'B5L15','B5L16','B2L30','B2L31','B2L32','A6R1','A6R2','A6R3','A3R8','A3R9','A3R10'] #'B2L31' is missing
T6correct = []
T7correct = []
            ...
            for filematch in corr_dict[tdays[i]]:
                if filematch=='B2L31':
                    ...
                    for fake in range(len(samdict)):
                        ...
                        dfn.insert(0,"sample_number",samdict[sampnum])
                        dfn.insert(0,"tree_id",tree_idx)
                        dfn.insert(0,"test_number",tdays[i])
                        mdf = pd.concat([mdf,dfn])
